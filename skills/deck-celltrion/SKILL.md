...
바이오시밀러와 신약으로 여는 글로벌 헬스케어를 담은 **Celltrion 브랜드 테마 PPTX** 생성기. 검증된 브랜드 컬러와
...
| role | hex | role | hex |
|---|---|---|---|
| accent | `#50B848` | bg | `#FFFFFF` |
| ink | `#1A1A1A` | surface | `#F5F6F8` |
| sub | `#4A4A4A` | border | `#E1E3E8` |
| divider_bg | `#00552D` | divider_ink | `#FFFFFF` |
...
python3 ../../_engine/render_deck.py \
  ...
  --out examples/celltrion-6p.pptx --pdf

# 3) 검증 게이트 (PASS라야 배포)
python3 ../../_engine/validate.py examples/celltrion-6p.pptx \
...
> ⚠️ **비공식 브랜드 영감 테마입니다.** 본 스킬은 Celltrion과 제휴·보증 관계가 없으며,
> 상표·로고는 셀트리온 (Celltrion, Inc.)의 자산입니다. 팔레트는 공개된 브랜드 컬러를
