...
은행·증권·보험을 아우르는 종합 금융 그룹를 담은 **KB Financial Group 브랜드 테마 PPTX** 생성기. 검증된 브랜드 컬러와
...
| role | hex | role | hex |
|---|---|---|---|
| accent | `#FFBC00` | bg | `#FFFFFF` |
| ink | `#1A1A1A` | surface | `#F5F6F8` |
| sub | `#4A4A4A` | border | `#E1E3E8` |
| divider_bg | `#2B2B2B` | divider_ink | `#FFFFFF` |
...
python3 ../../_engine/render_deck.py \
  ...
  --out examples/kb-financial-6p.pptx --pdf

# 3) 검증 게이트 (PASS라야 배포)
python3 ../../_engine/validate.py examples/kb-financial-6p.pptx \
...
> ⚠️ **비공식 브랜드 영감 테마입니다.** 본 스킬은 KB Financial Group과 제휴·보증 관계가 없으며,
> 상표·로고는 KB금융그룹 (KB Financial Group Inc.)의 자산입니다. 팔레트는 공개된 브랜드 컬러를
