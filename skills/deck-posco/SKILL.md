...
철강을 넘어 2차전지 소재로 확장하는 소재 기업를 담은 **POSCO 브랜드 테마 PPTX** 생성기. 검증된 브랜드 컬러와
...
| role | hex | role | hex |
|---|---|---|---|
| accent | `#053080` | bg | `#FFFFFF` |
| ink | `#1A1A1A` | surface | `#F5F6F8` |
| sub | `#4A4A4A` | border | `#E1E3E8` |
| divider_bg | `#053080` | divider_ink | `#FFFFFF` |
...
python3 ../../_engine/render_deck.py \
  ...
  --out examples/posco-6p.pptx --pdf

# 3) 검증 게이트 (PASS라야 배포)
python3 ../../_engine/validate.py examples/posco-6p.pptx \
...
> ⚠️ **비공식 브랜드 영감 테마입니다.** 본 스킬은 POSCO과 제휴·보증 관계가 없으며,
> 상표·로고는 포스코 (POSCO Holdings)의 자산입니다. 팔레트는 공개된 브랜드 컬러를
